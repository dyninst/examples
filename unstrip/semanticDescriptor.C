...
int SemanticDescriptor::count(SemanticDescriptorElem& elem, Database& db) {
  ...
  for(iter = _sd.begin(); iter != _sd.end(); ++iter) {
    if(!strcmp((char*)(*iter)[0], (char*)(elem[0]))) {
      if(elem.equals(*iter, db))
        ret++;
    }
  }
 ...
 */
int SemanticDescriptor::closerMatch(SemanticDescriptor& a, SemanticDescriptor& b, Database& db) {
  double coverageA = a.coverage(*this, db);
  double coverageB = b.coverage(*this, db);

  if(coverageA > coverageB)
    return 1;
  else if(coverageB > coverageA)
    return -1;
  else {
    if(a.size() < b.size())
      return 1;
