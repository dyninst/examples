...
bool DescriptorDatabase::build() {
  ...
  while(!feof(descriptorFile)) {
    ...
    if(fgets(cur, MAXLEN, descriptorFile) == NULL)
      break;
    ...
    for(trapSetIter = trapSets.begin(); trapSetIter != trapSets.end(); ++trapSetIter) {
...
bool SyscallParamDatabase::build() {
  ...
  while(!feof(paramFile)) {
    if(fgets(buf, 128, paramFile) == NULL)
      break;
...
bool SyscallNumbersDatabase::build() {
  ...
  while(!feof(unistdFile)) {
    if(fgets(buf, 128, unistdFile) == NULL)
      break;
 ...
 */
bool Database::setup(SymtabAPI::Symtab* symtab, Mode mode, string relPath) {
  bool ret = true;
