...
int main(int argc, const char* argv[]) {
  if(argc != 3) {
    std::cerr << "Usage:\n\t" << argv[0] << " <input binary> <output binary path>\n";
    return -1;
  }
  ...
  for(dpa::PatchFunction* fun : functions) {
    ...
    patchMgr->findPoints(dpa::Scope(fun), dpa::Point::FuncEntry, back_inserter(f_entryPoints));

    std::cout << fun->name() << " has:\n\t" << f_entryPoints.size() << " entry points" << std::endl;
