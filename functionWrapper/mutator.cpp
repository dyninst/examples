...
bool wrap(string original, string wrapper, string clone) {
  ...
  ParseAPI::CodeObject *co = Dyninst::ParseAPI::convert(w[0])->obj();
  auto *src = dynamic_cast<ParseAPI::SymtabCodeSource *>(co->cs());
  if (!src) {
...
bool parseOptions(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "i:o:l:s:h")) != -1) {
    ...
    case 'l': {
      if (!app) {
        cerr
            << "Error: must specify input file before loading wrapper libraries"
            << endl;
        exit(0);
      }
