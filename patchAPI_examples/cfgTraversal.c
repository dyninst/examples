...
for (std::vector<PatchFunction*>::iterator fi = all.begin();
  ...
  for (auto  bi = blks.begin(); bi != blks.end(); bi++) {
    // Print out each block's size
    PatchBlock* blk = *bi;
    std::cout << "\tBlock size:" << blk->size() << std::endl;


    std::vector<PatchEdge*> edgs;
    obj->edges(back_inserter(edgs));

    for (std::vector<PatchEdge*>::iterator ei = edgs.begin()) {

    }
  }
}

	PatchEdge * edge = obj->edges();
